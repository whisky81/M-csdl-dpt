...
---------------------------------------------------------
  ...
  8. ensure_indexes() — 4 ivfflat indexes for all (norm × metric) combos
"""
...
def _vec_to_list(v) -> List[float]:
    ...
    Workaround: fetch as TEXT ('::text') and parse '[x,y,z]' string here.
    Also handles the case where pgvector Python bindings return ndarray/list.
    """
    if v is None:
...
# ===========================================================================
# 1.  Worker function — MODULE LEVEL so ProcessPoolExecutor can pickle it
# ===========================================================================
def _extract_worker(file_path: str) -> Optional[List[float]]:
    ...
    try:
        ...
        silence_ratio = float(np.sum(rms_db < SILENCE_DB) / rms.shape[1]) \
                        if rms.shape[1] > 0 else 0.0
...
class DBPool:
    ...
    USAGE — ALL cursor operations must stay inside the 'with' block:

        with db.conn() as conn:
            cur = conn.cursor()
            cur.execute(...)       # ← inside block
            rows = cur.fetchall()  # ← inside block
        # conn is committed & returned to pool here
    """
    ...
    @contextmanager
    def conn(self):
        """Yield a connection; always commit + return it to the pool on exit."""
...
class FeatureExtractor:
    ...
    def _fetch_paths(self) -> List[Tuple[int, str]]:
        with self.db.conn() as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT obj_id, file_path FROM multimedia_objects ORDER BY obj_id;"
            )
            return cur.fetchall()
    ...
    async def process_all(
        ...
        log.info("Extracting %d files — %d workers, batch=%d …",
...
# ===========================================================================
class Normalizer:
    """Store raw vectors, compute norm params, apply normalization, build indexes."""
    ...
    def ensure_schema(self):
        """Add vector columns and normalization_params table if missing."""
        with self.db.conn() as conn:
            cur = conn.cursor()
    ...
    # ── ivfflat indexes — 4 combos: (minmax|zscore) × (l2|cosine) ───────────
    def ensure_indexes(self):
        """
        Create 4 ivfflat indexes for approximate nearest-neighbour search.
        Uses CREATE INDEX IF NOT EXISTS — safe to call repeatedly.

        Index          | Column               | Operator class
        ───────────────|──────────────────────|─────────────────
        idx_mm_l2      | min_max_scaling_vec  | vector_l2_ops
        idx_mm_cos     | min_max_scaling_vec  | vector_cosine_ops
        idx_zs_l2      | z_score_nor_vec      | vector_l2_ops
        idx_zs_cos     | z_score_nor_vec      | vector_cosine_ops
        """
        indexes = [
            ("idx_mm_l2",  "min_max_scaling_vec", "vector_l2_ops"),
            ("idx_mm_cos", "min_max_scaling_vec", "vector_cosine_ops"),
        ...
        with self.db.conn() as conn:
            cur = conn.cursor()
            for name, col, ops in indexes:
                cur.execute(f"""
                    CREATE INDEX IF NOT EXISTS {name}
                    ON multimedia_objects
                    USING ivfflat ({col} {ops})
                    WITH (lists = 100);
                """)
                log.info("Index %s ready.", name)
        log.info("All 4 ivfflat indexes created.")

    # ── Bulk-store raw vectors ───────────────────────────────────────────────
    def store_raw_vectors(self, data: List[Tuple[int, List[float]]]):
        """Single execute_values call — 1 DB round-trip for all rows."""
        if not data:
            return
    ...
    def compute_params(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        Fetch all raw vectors (as text to avoid pgvector CannotCoerce),
        return (min, max, mean, std) as numpy arrays of shape (FEATURE_DIM,).
        """
        with self.db.conn() as conn:
            cur = conn.cursor()
            cur.execute(
                # ::text workaround — pgvector has no direct cast to float8[]
        ...
        assert matrix.shape == (len(rows), FEATURE_DIM), \
            f"Unexpected shape {matrix.shape}"
        ...
        log.info("Params computed: %d samples × %d dims.", *matrix.shape)
    ...
    def apply_normalization(
        ...
        """
        NumPy broadcast over the whole chunk → no Python loops per dimension.
        Each chunk = 1 execute_values SQL call.
        """
        with self.db.conn() as conn:
            cur = conn.cursor()
            cur.execute(
        ...
        for i in range(n_chunks):
            ...
            log.info("  Chunk %d/%d — %d rows (total %d).",
...
async def main():
    ...
    try:
        ...
        # ── Step 2: skip extraction if all raw vectors already present ────
        with db.conn() as conn:
            cur = conn.cursor()
            cur.execute("""
                SELECT
        ...
        # ── Step 5: build 4 ivfflat indexes ──────────────────────────────
        normalizer.ensure_indexes()
